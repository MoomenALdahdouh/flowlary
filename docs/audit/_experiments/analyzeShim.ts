/**
 * EXPERIMENT SHIM ONLY.
 * Copy of extension/src/core/engine/chunks.ts plus the missing `corrected` binding
 * so the rest of the current engine can be evaluated. Not used by production.
 */
/**
...
import { scriptCounts } from '../../../extension/src/features/correction/language.ts'
...
import { inferLayoutSpans, layoutMappingCoverage } from '../../../extension/src/core/engine/layoutSequence.ts'
import { isStructuralTechnicalToken, isTechnicalToken, looksLikeIntentionalLatinInArabic } from '../../../extension/src/core/engine/technicalTokens.ts'
import type { AnalyzeOptions, SharedAnalysis, SpanRole, TextOrigin, WritingChunk } from '../../../extension/src/core/engine/types.ts'
...
export function analyzeFieldText(text: string, options: AnalyzeOptions = {}): SharedAnalysis {
  ...
  const chunks: WritingChunk[] = tokens.map((span, index) => {
    ...
    const covering = options.correctedRanges?.some((range) => overlaps(range, span.start, span.end))
      ? undefined
      : layoutSpans.find((item) => overlaps(item.range, span.start, span.end) || overlaps(item.range, span.rawStart, span.rawEnd))
    ...
    if (override) role = 'user_override'
    ...
    else if (layoutSuspicion !== 'none' && covering) role = 'possible_layout_error'
      ...
      else role = hasArabicNeighbor ? 'intentional_foreign_token' : 'unknown'
  ...
  const hasLayoutSuspicion = layoutSpans.length > 0 || chunks.some((chunk) => chunk.layoutSuspicion !== 'none')
