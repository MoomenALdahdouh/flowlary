...
describe('characterization: token protection', () => {
  it('skips emails, urls, secrets, and cards', () => {
    ...
    expect(skipReasonForToken('https:')).toBe('url')
    expect(skipReasonForToken('http')).toBe('url')
    expect(skipReasonForToken('www')).toBe('url')
    expect(skipReasonForToken(JWT)).toBe('jwt')
    expect(skipReasonForToken('sk-abcdefghijklmnopqrstuvwxyz123456')).toBe('api-key')
    expect(skipReasonForToken('sk')).toBe('api-key')
    expect(skipReasonForToken('sk-')).toBe('api-key')
    expect(skipReasonForToken('sk-ab')).toBe('api-key')
    expect(skipReasonForToken('eyJ')).toBe('jwt')
    expect(skipReasonForToken('ey')).toBe('jwt')
